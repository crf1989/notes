...
*** CL-PDF
    ...
    + The page sizes are store in ~config.lisp~:
      #+BEGIN_SRC lisp
      ...
      To set the page size, use the code like
      #+BEGIN_SRC lisp
        (pdf:with-page (:bounds #(0 0 595 841))
          ...)
      #+END_SRC
    + APS column width 8.5cm, the golden ratio height is 5.26cm
    + Dashed line
      
      http://www.verypdf.com/document/pdf-format-reference/pg_0218.htm

      For example, the code below set a dashed line with 3 pixels.
      #+BEGIN_SRC lisp
      (pdf:set-dash-pattern '(3) 0)
      #+END_SRC
    + Filled Circle
      #+BEGIN_SRC lisp
        (pdf:circle 10 10 5)
    ...
    + Arrows 
      ...
      Need to write a function by hand, see
      https://www.pdflib.com/pdflib-cookbook/graphics/arrows/
** Gnu AWK
...
** [[http://www.a4papersize.org/][A4paper size]]
   ...
   + A4 paper size in pixels:
     ...
     If 72 DPI is used, 1cm = 28.33333333333333 pixels.
* Mathematics 
** Useful formulas
*** Gaussian Integrals
